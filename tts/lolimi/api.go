...
const (
	...
	dxlURL     = lolimiURL + "/API/yyhc/dxl.php?msg=%v"
	lyyURL     = lolimiURL + "/API/yyhc/lyy.php?msg=%v"
	kbURL      = lolimiURL + "/API/yyhc/kb.php?msg=%v"
	sxcURL     = lolimiURL + "/API/yyhc/sxc.php?msg=%v"
	czURL      = lolimiURL + "/API/yyhc/cz.php?msg=%v"
	dzURL      = lolimiURL + "/API/yyhc/dz.php?msg=%v"
)

var (
	lolimiMap = map[string]string{
		"嘉然":  jiaranURL,
		"塔菲":  tafeiURL,
		"东雪莲": dxlURL,
		"懒羊羊": lyyURL,
...
func TTS(name string, text string) (recURL string, err error) {
	...
	if !ok {
		...
		ttsURL = fmt.Sprintf(ttsURL, text)
	}
	data, err = web.GetData(ttsURL)
	if err != nil {
		return
	}
	recURL = gjson.Get(binary.BytesToString(data), "music").String()
	return
}
